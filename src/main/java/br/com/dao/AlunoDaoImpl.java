...
public class AlunoDaoImpl extends BaseDaoImpl<Aluno, Long> implements AlunoDao{
    ...
    public List<Object[]> listarRankingMes(Date data, long idTurma, Session sessao) throws HibernateException {
        ...
        Query consulta = sessao.createQuery("from Aluno aluno JOIN aluno.emprestimos AS emprestimo where aluno.turma.id = :idTurma and year(emprestimo.dataRetirada)=year(:data) and month(emprestimo.dataRetirada)=month(:data) group by aluno.id order by count(emprestimo.aluno.id) DESC");
